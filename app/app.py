...
def preprocess_item(item: Item) -> pd.DataFrame:
    ...
    cat_transformed = pd.DataFrame(ohe.transform(data[cat_cols]),
...
def predict_file(file: UploadFile) -> str:
    ...
    df['engine'] = df['engine'].fillna(df['engine'].median()).astype(float).astype(int)
    ...
    cat_transformed = pd.DataFrame(ohe.transform(df[cat_cols]),
                                   columns=ohe.get_feature_names_out(cat_cols),
                                   index=df.index)
